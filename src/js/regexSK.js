...
function isValidPassword(inputElement) {
    ...
    return validateInput(
        ...
        'Heslo musí obsahovať aspoň jedno veľké písmeno, jedno malé písmeno a jednu číslicu.'
    );
}
...
function isValidText(inputElement) {
    ...
    return validateInput(
        inputElement,
        regex,
        'Toto pole nemôže byť prázdne.',
        'Zadajte platný reťazec začínajúci veľkým písmenom.'
    );
}
...
function isValidQuestion(inputElement) {
    ...
    return validateInput(
        inputElement,
        regex,
        'Otázka nemôže byť prázdna.',
        'Zadajte platnú otázku končiacu na "?".'
    );
...
function isValidKey(inputElement) {
    ...
    return validateInput(
        inputElement,
        regex,
        'Kľúč nemôže byť prázdny.',
        'Kľúč musí mať presne 5 znakov.'
    );
}
...
function checkForm(e) {
    ...
    if (formIsValid) {
        ...
        toastr.error('Prosím, vyplňte formulár správne.');
        return false;
    }
}
...
function checkFormAdd() {
    ...
    if (type.value === '2') {
        ...
        if (!aSwitch.checked && !bSwitch.checked && !cSwitch.checked) {
            toastr.error('Vyberte správnu odpoveď.');
            return false;
        }
    }
